...
model_paramnames=Hippo('parameters');
...
targets = {'kf7'};%,'RSmad2','cAbl','Receptor2','kc151'
...
for j=1:length(targets)
    tmp_iniCond1 = tmp_initialConditions;
     ...
     for i= 1:length(tmp_dose)
        ...
        modelparamvals1(ismember(model_paramnames,targets{j})) = tmp_modelparamvals(ismember(model_paramnames,targets{j}))*tmp_dose(i);
        out=Hippo(tmp_simtime,tmp_initialConditions,modelparamvals1');
        tmp_fc_varvals=out.variablevalues(tmp_tidx,:);
        
        tmp_YAPSmad(i,:)=(tmp_fc_varvals(:,ismember(out.variables,"pSmad2YAPr")))';
    ...
    saveas(gcf,sprintf('figures/%ssmad_yap.png',targets{j}));   
     saveas(gcf,sprintf('figures/svg/%ssmad_yap.svg',targets{j}));      
 % __________________________________________
    ...
    saveas(gcf,sprintf('figures/%sp73_yap.png',targets{j}));
     saveas(gcf,sprintf('figures/svg/%sp73_yap.svg',targets{j}));   
